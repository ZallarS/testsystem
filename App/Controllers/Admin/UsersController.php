...
class UsersController extends Controller
{
    ...
    public function update($id)
    {
        try {
            ...
            if (!empty($validationErrors)) {
                return Response::redirect("/admin/users/edit/{$id}?error=" . urlencode(implode(', ', $validationErrors)));
            }
    ...
    private function handleException(\Exception $e, $defaultMessage = 'An error occurred')
    {
        error_log($e->getMessage());
        return Response::redirect('/admin/users?error=' . urlencode($defaultMessage));
    }
    ...
    protected function authorize($permission)
    {
        ...
        // Проверяем конкретные разрешения для пользователя
        switch ($permission) {
            case 'users.manage':
            case 'users.delete':
            case 'users.create':
